...
class LLMFactory:
    ...
    def _create_cerebras(
        model: str = None, temperature: float = 0.7, **kwargs
    ) -> BaseChatModel:
...
class LLMService:
    ...
    async def generate_questions(
        ...
        try:
            response = await chain.ainvoke(
                {
                    ...
                    "language_instruction": LANGUAGE_INSTRUCTIONS.get(
                        language, LANGUAGE_INSTRUCTIONS["gu"]
                    ),
                    "include_answers_instruction": "Include correct answers and explanations."
                    if include_answers
                    else "",
                }
            )
            ...
            logger.info(
                f"Generated {len(validated_questions)} questions using {self.provider}"
            )
